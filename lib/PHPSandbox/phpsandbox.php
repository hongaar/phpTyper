...
class PHPSandbox {
	...
	public function __construct($options = array()){
		if (isset($options['tmp_dir'])) {
			$this->options['tmp_dir'] = $options['tmp_dir'];
		}

		$this->options['chroot'] = $this->sys_get_temp_dir();
	...
	private function sys_get_temp_dir() {
		if (!empty($this->options['tmp_dir'])) { return realpath($this->options['tmp_dir']); }
	...
	private function buildCLIOptions(){
		$this->cli_options = '-d session.name=PHPSESSID -d chroot="'.addslashes($this->options['chroot']).'" -d display_errors='.$this->options['display_errors']. ' -d memory_limit='.$this->options['memory_limit'].' -d max_execution_time='.$this->options['max_execution_time'];
	...
	public function runFile($path, $pass_through_vars = array(), $lintCode = true){
		...
		if(file_exists($path)){
			if(($lintCode && $this->lintFile($path)) || !$lintCode){
				...
				$command = "$this->cli_command $this->cli_options ".$this->enhancedProtection($chroot)." -d chroot=\"".addslashes($chroot)."\" -f \"".addslashes($path)."\" ".$this->buildVars($pass_through_vars);
	...
	* This input will be stored as a file for future re-running, supports lint
	...
	private function lintFile($path){
		$output;
		$return_var;
		$command = $this->cli_command . ' -l -f ' . '"' . $path . '"';
	...
	private function createTempCHRoot($salt = ''){
		if(is_writable($this->sys_get_temp_dir())){
			$dir = $this->sys_get_temp_dir() . DIRECTORY_SEPARATOR . md5(uniqid(true).$salt);
	...
	private function enhancedProtection($scriptDir){
		...
		if($this->options['directory_protection']){
			...
			$str .= ' -d open_basedir="'.addslashes($this->tempPath);

			if($this->options['directory_protection_allow_tmp']){
				$str .= $dir_seperator . addslashes($this->tempPath);
			}

			if($this->options['pass_session_id']){
				$str .= $dir_seperator . addslashes(session_save_path());
			}
